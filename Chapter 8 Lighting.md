...
为了给粗糙度数学建模，我们需要使用到微平面这个模型，即认为一块微小的平面由多块光滑的平面构成。同时，这些光滑的平面的法线，就是我们之前所说的微观上的法线。假设有一个视点$v$以及一条光线$L$。我们想知道有多少光滑的平面能够将光线$L$反射到视点$v$，换句话说就是有多少的光滑平面它的法线$h = normalize(L + v)$。观察[图片8.18](#Image8.18)，就是一个能够将光线$L$反射到视点$v$的光滑的平面。
...
因为$h$和$n$都是单位向量，因此$cos(\theta_h) = (n \cdot h)$是相等的。[图片8.19](#Image8.19)则显示了$m$取不同的值的时候，函数的图像。随着$m$的减少，平面就越来越粗糙，微面的法线逐渐和平面的法线分离。而随着$m$的增加，平面变的越来越光滑，微面的法线逐渐偏向于平面的法线。
