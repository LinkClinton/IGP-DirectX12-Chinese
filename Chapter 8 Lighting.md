...
$$LitColor = A_L \otimes m_d + max(L \cdot n, 0) \cdot B_L \otimes \bigg( m_d + R_F(\alpha_h) \frac{m + 8}{m}(n \cdot h)^m \bigg) [eq8.4]$$
...
手电筒是一个典型的聚光灯的例子。对于聚光灯来说，其由光的位置$Q$以及它的朝向$d$构成。它的辐射范围则是一个锥体(参见[图片8.27](#Image9.27))。

![Image8.27](Images/8.27.png)

对于聚光灯来说，我们使用和点光源一样的方法:

$$L = \frac{Q - P}{\|Q - P\|}$$

$P$是计算光照的点，$Q$是聚光灯的位置。观察[图片8.27](#Image8.27)，只有当点$P$在聚光灯锥形范围内的时候，才计算光照。换句话说，只有$-L$和$d$之间的夹角$\Phi$比锥体的角度$\Phi_{max}$小，才计算光照。

当然，在聚光灯锥体内部的点受到的光照不是相等的，在锥体中间的点应该受到的光照的强度，然后随着角度逐渐从0增加到$\Phi_{max}$逐渐变为0。

因此我们需要同样需要一个函数来模拟光的强度随着$\Phi$的变化而变化。我们使用一个和[图片8.19](#Image8.19)一样的函数来模拟。只不过将$\theta_h$替换成$\Phi$，将$m$替换成$s$。
