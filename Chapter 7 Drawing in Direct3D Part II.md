...
但是如果你在着色器中使用了一个参数，而没有在来源标记中声明的话，肯定会报错(会在创建渲染管道状态的时候报错，被坑过好几次了)。
...
主要来说就是我们创建一个较大的来源标记，能够兼容我们的不同的渲染管道状态使用的着色器，也就是说可能我们要使用的着色器未必会全部使用到我们在来源标记声明的资源，但是不同的着色器需要的资源可能不同。因此创建一个能够兼容所有着色器的来源标记的话就不需要切换来源标记了。当然你需要注意的是，如果这个来源标记太大了，可能我们得到的性能减损还不如由于太大导致的性能消耗多，从而导致根本没有得到优化。
