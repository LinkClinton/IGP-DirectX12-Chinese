...
我们把一个顶点和摄像机的位置的连线叫做这个点的投影线，那么我们就可以得到透视投影的变换就是将顶点**v**变换到投影线和投影平面的交点**v'**，我们就称 **v'** 为**v**的投影。
...
由于我们之后要将投影平面的内容映射到后台缓冲中去，因此我们需要确定投影平面的长宽比要和后台缓冲一致。
因此我们可以认为后台缓冲的大小将会决定投影平面的长宽比。例如后台缓冲的大小是`800x600`，那么我们就设`r`为`r = 800/600 ≈ 1.333`
...
参见图片[5.23](#Image5.23)，我们首先可以知道投影平面的大小并不重要，为了方便我们就认为投影平面的高度为2，宽度的话就可以通过长宽比来得到。

<img src="Images/5.23.png" id = "Image5.23"> </img>

为了保证水平视野角度`α`，因此投影平面到原点的距离`d`并不是随意的，我们需要计算出来。

**<center>tan (α/2) = 1 / d => d = cot (α/2)</center>**

因此我们就可以算出投影平面(**高度为2**)到原点的距离`d`了。
我们因此也可以算出`β`:
...
因此如果一个点他在这个平截头体里面的话，那么它投影后的点必然在投影平面内，当然这个点的Z轴坐标必然要在 **(n, f)** 内。

#### <element id = "5.6.3.3"> Normalized Device Coordinates (NDC) </element>

我们在先前的章节计算了一个顶点投影后的坐标。
并且我们知道在视角空间中，投影平面的高度是**2**，他的宽度是**2r**(`r`是长宽比)。
也就是说投影平面的大小就由`r`来决定。
因此我们的硬件在处理关于投影平面的操作(例如将投影平面的内容映射到后台缓冲中去)的时候是肯定需要知道`r`的，因此我们需要告诉硬件`r`的值。
...
我们可以知道在NDC空间中，投影平面就变成了一个宽度和高度都为2的正方形了，即我们确定了他的尺寸。
